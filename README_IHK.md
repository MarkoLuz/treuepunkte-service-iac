...
To prevent this, idempotency is used so that each request is processed only once.
...
- In AWS, a dedicated Lambda function (`schema-init`) initializes the schema during deployment using a CloudFormation custom resource
...
The system was validated locally and in AWS.
